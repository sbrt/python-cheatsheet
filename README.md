...
<list> = [None] * <int>        # initialize empty list of given length
```
...
```python
<dict> = dict(<collection>)                     # Creates a dict from coll. of key-value pairs. Or: <dict> = dict([(<key>,<value>), ...]) 
...
indexes_sorted   = sorted(enumerate(<collection>), key=operator.itemgetter(1))
